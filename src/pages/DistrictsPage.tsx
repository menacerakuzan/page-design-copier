                  ...
                  <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    {districts.map((d, idx) => (
                      <motion.button key={d.id} type="button" onClick={() => selectDistrict(d)}
                        ...
                        {d.imageUrl ? (
                          <Img w={500} src={d.imageUrl} alt={d.name} className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
                        ) : (
                  ...
                  <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                    {districtCities.map((c, idx) => (
                      <motion.button key={c.id} type="button" onClick={() => selectCity(c)}
                        ...
                        {c.imageUrl ? (
                          <Img w={500} src={c.imageUrl} alt={c.name} className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
                        ) : (
